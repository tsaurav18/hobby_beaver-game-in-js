...
const fg = {
    ...
    update: function(){  //earth moving forwards
...
if(state.current == state.getReady){
    this.y = 150; //reset the position of the game after game over
...
//if the speed is greater than jums means the bird is falling down
...
const pipes ={
    ...
    update: function(){
        ...
        for(let i=0;i<this.position.length;i++){
           ...
           p.x -= this.dx;
            //if the pipes go byeond canvas, we delete them from the array
            if(p.x+ this.w<=0){
                this.position.shift();
                score.value+= 1; 
